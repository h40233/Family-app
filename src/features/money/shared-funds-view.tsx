...
export function SharedFundsView() {
  ...
  async function loadFamiliesAndFunds(nextSelectedFundId?: string) {
    ...
    if (nextFundId) {
      await loadTransactions(family.id, nextFundId);
    } else {
      setTransactions([]);
    }
  }
  ...
  async function loadTransactions(nextFamilyId: string, fundId: string) {
    const loadedTransactions = await apiRequest<FundTransaction[]>(
      `/api/v1/families/${nextFamilyId}/funds/${fundId}/transactions`
    );
  ...
  useEffect(() => {
    ...
    async function load() {
      if (!online) {
        setLoading(false);
        setPermissionNotice("共用基金屬於協作資料，需連線後操作。");
        return;
      }
      ...
      try {
        ...
        if (!cancelled) {
          if (loadError instanceof ApiClientError && loadError.status === 403) {
            setPermissionNotice("你目前沒有權限查看這個家庭的共用基金。");
          } else {
  ...
  async function handleFundChange(fundId: string) {
    ...
    if (!online) {
      setPermissionNotice("共用基金需連線後才能查看交易與操作。");
      return;
    }
    ...
    try {
      ...
      if (loadError instanceof ApiClientError && loadError.status === 403) {
        setPermissionNotice("你目前沒有權限查看這筆基金的交易紀錄。");
      } else {
        setError(errorMessage(loadError));
      }
    }
  ...
  async function handleCreateFund(event: FormEvent<HTMLFormElement>) {
    ...
    if (!online) {
      setPermissionNotice("建立共用基金需連線後操作。");
      return;
    }
    ...
    try {
      const fund = await apiRequest<SharedFund>(`/api/v1/families/${familyId}/funds`, {
        method: "POST",
      ...
      if (saveError instanceof ApiClientError && saveError.status === 403) {
        setPermissionNotice("你目前沒有權限建立共用基金。");
      } else {
  ...
  async function handleCreateTransaction(event: FormEvent<HTMLFormElement>) {
    ...
    if (!online) {
      setPermissionNotice("共用基金是協作資料，需連線後操作。");
      return;
    }
    ...
    try {
      await apiRequest<FundTransaction>(
        `/api/v1/families/${familyId}/funds/${selectedFund.id}/transactions`,
        {
          method: "POST",
          body: JSON.stringify({
            type,
      ...
      if (saveError instanceof ApiClientError && saveError.status === 403) {
        setPermissionNotice("你目前沒有權限新增這筆基金的存入或支出。");
      } else {
        setError(errorMessage(saveError));
      }
    } finally {
  ...
  return (
    <>
      <PageHeader
        eyebrow="共用基金"
        title="家庭基金與交易"
        description={`${familyName} 的共用基金紀錄。共用基金牽涉多人協作，離線時只能查看提示，需連線後才能操作。`}
        action={
          <button
            type="button"
            disabled={!online || loading}
            onClick={() => void loadFamiliesAndFunds(selectedFundId)}
          >
            重新整理
          </button>
        }
      ...
      {(!online || permissionNotice) && (
        <section className="panel" style={{ marginBottom: "1rem" }}>
          <strong>{online ? "權限提示" : "需連線後操作"}</strong>
          <p className="muted">
            {permissionNotice || "共用基金是協作資料，請連線後再新增基金或交易。"}
          </p>
        </section>
      )}
        ...
        <section className="panel">
          ...
          <div className="module-list">
            {funds.map((fund) => (
              <button
                key={fund.id}
                type="button"
                onClick={() => void handleFundChange(fund.id)}
                disabled={!online}
                style={{
                  background: fund.id === selectedFundId ? "var(--primary)" : "var(--surface)",
                  border: "1px solid var(--border)",
                  color: fund.id === selectedFundId ? "white" : "var(--text)",
                  textAlign: "left"
                }}
              >
          ...
          <form onSubmit={(event) => void handleCreateFund(event)} style={{ marginTop: "1rem" }}>
            ...
            <div style={{ display: "grid", gap: "0.75rem" }}>
              <input name="name" placeholder="基金名稱，例如家庭日常基金" />
              <button type="submit" disabled={saving || !online || !familyId}>
                建立基金
              </button>
            </div>
          </form>
        ...
        <section className="panel">
          ...
          <form onSubmit={(event) => void handleCreateTransaction(event)}>
            <div style={{ display: "grid", gap: "0.75rem", marginBottom: "1rem" }}>
              <select name="type" defaultValue="expense" disabled={!online}>
                <option value="expense">支出</option>
                <option value="deposit">存入</option>
              </select>
              <input name="amount" inputMode="decimal" placeholder="金額" disabled={!online} />
              <input name="category" placeholder="分類，例如餐飲、家用" disabled={!online} />
              <input name="note" placeholder="備註" disabled={!online} />
              <button type="submit" disabled={saving || !online || !selectedFund}>
                新增基金交易
              </button>
            </div>
          </form>
          ...
          <div className="module-list">
            ...
            {transactions.length === 0 && <p className="muted">目前基金尚無交易。</p>}
          </div>
        </section>
      </div>
    </>
